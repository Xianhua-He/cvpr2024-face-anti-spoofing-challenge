...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 pretrained_window_size=0, extra_norm_period=0):
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 extra_norm_period=0, fp16=False, **kwargs):
        super().__init__()
        ...
        #                               nn.BatchNorm1d(512, eps=1e-05),)
        # nn.init.constant_(self.output_layer[-1].weight, 1.0)
        ...
        # self.output_layer = nn.Sequential(
        #     nn.Flatten(),
        #     nn.Linear(in_features=self.num_features * 49, out_features=self.num_features, bias=False),
        #     nn.BatchNorm1d(num_features=self.num_features, eps=2e-5),
        #     nn.Linear(in_features=self.num_features, out_features=512, bias=False),
...
def swin_v2_t(num_classes, fp16=False):
    img_size = 224
    patch_size = 4
    in_chans = 3
    embed_dim = 96
...
def swin_v2_h(num_classes, fp16=False):
    img_size = 224
    patch_size = 4
    in_chans = 3
    embed_dim = 352
...
def swin_v2_g(num_classes, fp16=False):
    img_size = 224
    patch_size = 4
    in_chans = 3
    # embed_dim = 512
    ...
    if 'moco' in pretrain:
        ...
        for k in list(state_dict.keys()):
            ...
            if k not in model.state_dict().keys():
                print('key not exist {}'.format(k))
                continue
            if model.state_dict()[k].shape != state_dict[k].shape:
                print('skip {}, shape dismatch {} vs {}'.format(k, model.state_dict()[k].shape, state_dict[k].shape))
                del state_dict[k]
                continue
